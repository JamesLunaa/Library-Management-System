                ...
                <div class="card-body mainDisplay">
                    ...
                    <div id="book-list-margin" class="text-center mt-4 mx-4">
                        <div class="d-flex gap-2">
                            <form action="{{ route('instructorBookList') }}" class="flex-fill d-flex gap-2" method="post">
                                @csrf
                            ...
                            <a id="remove-reload" href="{{ route('instructor.searchBook') }}"><button type="submit"
                                    ...
                                    <div class='card ind text-center h-100 d-flex flex-column'>
                                        ...
                                        <div class="card-footer mt-auto" style="background-color: maroon">
                                            ...
                                            <h5 class="card-title" style="{{ $statusColor }}">
                                                {{ $books->status }}</h5>
                                            <form method="POST" action="{{ route('instructorBorrow') }}">
                                                @csrf
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#search-input').on('keyup', function () {
                ...
                $.ajax({
                    url: "{{ route('instructor.ajax.bookSearch') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        info: query
